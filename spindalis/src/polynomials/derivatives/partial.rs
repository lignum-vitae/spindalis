...
pub fn partial_derivative(poly: &Vec<Term>, var: &str) -> Vec<Term> {
    ...
    for part in poly {
        ...
        for i in 0..part.variables.len() {
            ...
            if v == var && *pow > 0 {
                // Power rule
                new_part.coefficient = part.coefficient * (*pow as f64);
                let new_power = pow - 1;
...
mod tests {
    ...
    fn test_partial_derivative_multiple_variables() {
        let poly = vec![Term {
            coefficient: 4.0,
            variables: vec![("x".into(), 2), ("y".into(), 3)],
        }];
        ...
        assert_eq!(result[0].variables, vec![("x".into(), 1), ("y".into(), 3)]);
    }
    ...
    fn test_partial_derivative_multiple_terms() {
        let poly = vec![
            ...
            Term {
                coefficient: 4.0,
                variables: vec![("x".into(), 1), ("y".into(), 1)],
            },
        ];
