...
impl<T> Arr2D<T> {
    ...
    /// Convert each element to another type `U` that T can convert into.
    pub fn convert_to<U>(&self) -> Arr2D<U>
    where
        T: Into<U> + Clone,
    {
        let inner: Vec<U> = self.inner.iter().cloned().map(|x| x.into()).collect();
        Arr2D {
            inner,
            height: self.height,
            width: self.width,
