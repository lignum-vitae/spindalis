...
use crate::polynomials::structs::extended::{ExtendedPolynomial, TokenStream};
...
#[allow(dead_code)]
fn parser(token_stream: Vec<Token>) -> Result<ExtendedPolynomial, PolynomialError> {
    let mut tokens = token_stream;
    implied_multiplication_pass(&mut tokens);
    let mut token_stream = tokens.into_iter().peekable();
    Ok(ExtendedPolynomial::new(parse_expr(&mut token_stream, 0.0)?))
}
...
mod tests {
    ...
    fn test_number_parse() {
        ...
        let expect = ExtendedPolynomial::new(Expr::Number(4.0));
    ...
    fn test_variable_parse() {
        ...
        let tok_str = lexer(expr).unwrap();
        let result = parser(tok_str).unwrap();
        let expect = ExtendedPolynomial::new(Expr::Variable("x".into()));
        assert_eq!(result, expect);
    }
    ...
    fn test_exponents_parse() {
        ...
        let tok_str = lexer(expr).unwrap();
        let result = parser(tok_str).unwrap();
        let expect = ExtendedPolynomial::new(Expr::BinaryOp {
            op: Operators::Caret,
    ...
    fn test_complex_expression_parse() {
        ...
        // (4x + 2) - (...)
        let expected = ExtendedPolynomial::new(Expr::BinaryOp {
            op: Operators::Sub,
    ...
    fn test_zero_parse() {
        ...
        let tok_str = lexer(expr).unwrap();
        let result = parser(tok_str).unwrap();
        let expect = ExtendedPolynomial::new(Expr::Number(0.0));
        assert_eq!(result, expect);
    }
